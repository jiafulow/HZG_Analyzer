      subroutine mrst2004f4(x,q,mode,upv,dnv,usea,dsea,str,chm,bot,glu)
C***************************************************************C
C                                                               C
C  This is a package for a four-fixed-flavour version of the    C
...
C                                                               C
C  Mode=1 gives a f=4 NLO set with Lambda(4) = 347 MeV          C
C  This set reads a grid called mrst2004f4nlo.dat               C
C  whose first row is 0.00910 ... 0.95443                       C
C                                                               C
C  Mode=2 gives a f=4 LO set with Lambda(4) = 220 MeV           C
C  This set reads a grid called mrst2004f4lo.dat                C
C  whose first row is 0.01580 ... 0.83473                       C
C                                                               C
C  Note that both these sets return zero for the                C
C  bottom distribution.                                         C
C                                                               C
...
c      if(x.lt.xmin.or.x.gt.xmax)       print 98,x
          if(mode.eq.1) then
        call mrst1f4(x,q2,upv,dnv,usea,dsea,str,chm,bot,glu) 
      elseif(mode.eq.2) then
        call mrst2f4(x,q2,upv,dnv,usea,dsea,str,chm,bot,glu) 
      endif 
  ...
  98  format('  WARNING:   X  VALUE IS OUT OF RANGE   ','x= ',e10.5)
      ...
      subroutine mrst1f4(x,qsq,upv,dnv,usea,dsea,str,chm,bot,glu)
      implicit real*8(a-h,o-z)
     ...
     .        1.8d6,3.2d6,5.6d6,1d7/
      ...
      if(init.ne.0) goto 10
        filename=checkpath('../hzgammaME/Pdfdata/mrst2004f4nlo.dat')
...
c notation: 1=uval 2=val 3=glue 4=usea 5=chm 6=str 7=btm 8=dsea
  ...
  40  continue
   ...
   50 format(8f10.5)
      ...
      subroutine mrst2f4(x,qsq,upv,dnv,usea,dsea,str,chm,bot,glu)
      implicit real*8(a-h,o-z)
      parameter(nx=49,nq=37,np=8,nqc0=2,nqb0=11,nqc=35,nqb=26)
      real*8 f1(nx,nq),f2(nx,nq),f3(nx,nq),f4(nx,nq),f5(nx,nq),
     .f6(nx,nq),f7(nx,nq),f8(nx,nq),fc(nx,nqc),fb(nx,nqb)
     ...
     .        1.8d6,3.2d6,5.6d6,1d7/
      ...
      q2save=qsq
      if(init.ne.0) goto 10
        filename=checkpath('../hzgammaME/Pdfdata/mrst2004f4lo.dat')
        open(unit=33,file=filename,status='old')
        do 20 n=1,nx-1
        do 20 m=1,nq
        read(33,50)f1(n,m),f2(n,m),f3(n,m),f4(n,m),
