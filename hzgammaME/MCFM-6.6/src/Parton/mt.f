      ...
      if(mode.eq.1)then
        filename=checkpath('../hzgammaME/Pdfdata/MTS1.DAT')
        open(unit=1,file=filename,status='old')
      elseif(mode.eq.2)then
        filename=checkpath('../hzgammaME/Pdfdata/MTE1.DAT')
        open(unit=1,file=filename,status='old')
      elseif(mode.eq.3)then
        filename=checkpath('../hzgammaME/Pdfdata/MTB1.DAT')
        open(unit=1,file=filename,status='old')
      elseif(mode.eq.4)then
        filename=checkpath('../hzgammaME/Pdfdata/MTB2.DAT')
        open(unit=1,file=filename,status='old')
      elseif(mode.eq.5)then
        filename=checkpath('../hzgammaME/Pdfdata/MTSN1.DAT')
        open(unit=1,file=filename,status='old')
      endif
