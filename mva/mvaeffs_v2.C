...
class StatDialogMVAEffs {  
   ...
   StatDialogMVAEffs(const TGWindow* p, Float_t ns, Float_t nb, string _log, string _varList);
...
StatDialogMVAEffs::StatDialogMVAEffs(const TGWindow* p, Float_t ns, Float_t nb, string _log, string _varList) :
   fNSignal(ns),
   fNBackground(nb),
   fFormula(""),
   fInfoList(0),
   ...
   varList(_varList)
{
  cout<<_log.c_str()<<endl;
  log.open(_log.c_str(),ios::in | ios::out | ios::app);
   //log.open(_log.c_str());
...
void mvaeffs_v2( TString fin = "TMVA.root", string _log = "log", string _varList = "vars",
              Bool_t useTMVAStyle = kTRUE, TString formula="S/sqrt(S+B)" )
{
   ...
   float totalSig = 6;
   float totalBG = 12000;
   StatDialogMVAEffs* gGui = new StatDialogMVAEffs(gClient->GetRoot(), totalSig, totalBG, _log, _varList);
