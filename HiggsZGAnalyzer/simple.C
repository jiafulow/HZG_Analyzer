...
void simple::Begin(TTree * /*tree*/)
{
  ...
  histoFile = new TFile("simpleHistograms_newTuple.root", "RECREATE");
  histoFile->cd();
...
{
  hm->fill1DHist(zga.costheta_lp,"h1_costhetaLP_newTuple", "Cos(#theta) positive lepton;cos(#theta);N_{evts}", 50, -1.1, 1.1, eventWeight);     
  hm->fill1DHist(zga.costheta_lm,"h1_costhetaLM_newTuple", "Cos(#theta) negative lepton;cos(#theta);N_{evts}", 50, -1.1, 1.1, eventWeight);     
  hm->fill1DHist(zga.phi,"h1_phi_newTuple", "#phi positive lepton;#phi;N_{evts}", 50, -3.2, 3.2, eventWeight);     
  hm->fill1DHist(zga.cosTheta,"h1_costhetaZG_newTuple", "Cos(#Theta) ZG system;cos(#Theta);N_{evts}", 50, -1.1, 1.1, eventWeight);     
  hm->fill1DHist(zga.costheta_lm+zga.costheta_lp,"h1_costhetaBoth_newTuple", "Cos(#theta) of both lepton;cos(#theta);N_{evts}", 50, -1.1, 1.1, eventWeight);     
}
/*
