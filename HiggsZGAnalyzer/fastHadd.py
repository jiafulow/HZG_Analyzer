...
def fastHadd():
  ...
  os.system('grep "eventNumber" ~/BatchOutput/%s/*/res/*.stdout  > dumps/DUMP_%s.txt' % (leptonB,tag))
  os.system('grep  "SASHA" ~/BatchOutput/%s/*/res/*.stdout  >> dumps/DUMP_%s.txt' % (leptonB,tag))
